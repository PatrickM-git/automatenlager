...
## Stand: 2026-05-13 (Session 6 – Mail-Dedup + Auto-Refill, Test-Suite 21/21)
...
Im bestehenden `Code - Offene Hinweise vorbereiten` greift zusätzlich der
`isFromToday(created_at)` Filter — alte Warnings tauchen am Folgetag
...
### Aktuelle Test-Suite (21 Tests, alle grün)
...
- T08g Slot current_qty ≤ aktive Lager-Summe
...
| Situation | Email? |
|---|---|
| Slot leer (Sheet sagt 0) | **Nein** (Auto-Refill heilt's) |
| Lager komplett leer (`EMPTY_BATCH`) | **Ja** — einmal, dann 7 Tage Ruhe |
| Lager unter Schwellwert (`LOW_BATCH`) | **Ja** — einmal, dann 7 Tage Ruhe |
| MHD läuft in ≤30 Tagen | **Ja** — einmal pro Charge, dann 7 Tage Ruhe |
| MHD abgelaufen | **Ja** — kritisch, ggf. erneut bis resolved |
| UNKNOWN_PRODUCT bei Verkauf | **Ja** — einmal, dann gilt Dedup |

### Vorheriger Teil derselben Session

### Nayax-Endpoint-Probe (10 Endpoints getestet)

Nur `/operational/v1/machines/{id}/lastSales` liefert 200. Die anderen 9
(`currentInventory`, `productMatrix`, `picklists`, `refills`, `events`,
`inventory`, `slots`, `planograms`, `snapshots`) geben **404 Not Found**.

→ Nayax meldet keinen Slot-Stand fuer die CR-FCRvMMfAiraF-Maschine.

### Konzept: Auto-Refill-Inferenz im WF3 (Modell A)

User-Anforderung: Auffuell-Vorgang am Automaten soll NICHT manuell ins Sheet
nachgepflegt werden muessen. Slot-Updates duerfen Lager NICHT beeinflussen.

Modell-Semantik (unveraendert, war schon korrekt):
- `Lagercharge.remaining_qty` = TOTAL physisch (Slot + Backstock)
- `Produkte.current_machine_qty` = nur Slot (Subset)
- Slot-Updates → Lager bleibt unveraendert (matchingColumns vermeidet das)
...
Bei jedem Sale:
  Wenn current_machine_qty <= 0 UND Lager.remaining > 0:
    → infer: User hat Slot aufgefuellt
  ...
  Wenn current_machine_qty <= 0 UND Lager.remaining = 0:
    ...
    → Email-Alert: "Bitte neue Lagercharge ergaenzen"
```
...
Resultat: Der User muss NIE den Slot manuell synchronisieren. Auto-Refill
heilt Slot=0 beim ersten Sale nach dem Auffuellen.

### WF5 LOW_STOCK-Severity downgraded zu `info`

Da Auto-Refill in WF3 jeden Slot=0 beim naechsten Sale heilt, ist der
LOW_STOCK-Alert in WF5 nicht mehr Email-wuerdig. Severity = `info` statt
`warning/critical`. EMPTY_BATCH und MHD_WARNING bleiben unveraendert
(echte Lager-Probleme).

Patch: `guv_check_tmp/patch_wf5_quiet_low_stock.js`

### Test-Suite erweitert (21 Tests, 20 PASS / 1 todo)

Neue Tests:
- T08g Slot current_qty <= aktive Lagercharge-Summe (zeigt Slot-Lager-Drift)
- T09c FIFO-Code enthaelt Auto-Refill-Inferenz (PATCH AUTO_REFILL_SLOT)

T08g ist aktuell rot fuer **SKU_FALCONE_NUSSNOUGAT** (Slot 19 > Lager 11).
Das ist kein WF-Bug — die Slot-Werte stammen aus alten BACKFILL-capacity-Init.
Wird durch User-Inventur (Inventur_2026-05-13_v2) gefixt.

### Inventur-Diagnose / Inventur-Tools (bereits in Session 6, frueh)

Diagnose ergab: 34 von 46 Lagerchargen kommen aus manueller Inventur 02.05.
(supplier=`Bestandsaufnahme...`). `initial_qty` ist KEINE
Rechnungs-Lieferungsmenge, sondern manuelle Bestandsaufnahme.

Tool `guv_check_tmp/inventory_export.js` legt im Sheet einen neuen Tab
`Inventur_<datum>_v2` an. Tool `guv_check_tmp/inventory_apply.js` appliert
User-eingaben (`real_total`, `real_im_slot`) zurueck.

User-Korrektur Snickers: Handschrift 17 wurde versehentlich als 77 gelesen.
init=77 → 17, remain auto korrekt 3. Konsistent.

Haribo Goldbaeren: Inkonsistenz Slot=8 / Lager=0 erkannt, Charge reaktiviert
(init=8, remain=8, mhd=2026-12-31). Best-guess, User soll verifizieren.

### Refill-Workflow fuer User (vereinfacht)

Du fuellst den Automaten auf — **keinerlei Sheet-Pflege noetig**:

| Aktion | Was passiert |
|---|---|
| Slot vom Backstock auffuellen | Nichts im Sheet noetig. WF3 erkennt es beim naechsten Sale automatisch und setzt `current_machine_qty` zurueck auf MIN(capacity, lager). |
| Neue Lieferung (Metro, Sonderposten) | WF1 mit Rechnungs-PDF → WF2 Approve → neue Lagercharge wird angelegt. |
| Manuelle Slot-Korrektur (z.B. Inventur) | Inventur-Sheet `Inventur_<datum>_v2` ausfuellen + `inventory_apply.js _v2`. |
| Email-Alert "Lager leer" | Echtes Problem — eine Charge muss ergaenzt werden. |
| Email-Alert "Slot leer" | NICHT mehr severity warning/critical (Auto-Refill heilt's). |

### Vorheriger Teil derselben Session

### Diagnose: Herkunft der `initial_qty`-Werte

**Ursprung:** ALLE Chargen mit `supplier='Bestandsaufnahme...'` (34 von 46 Chargen)
stammen aus einer manuellen Inventur am 2026-05-02 (Handschrift). Die `initial_qty`
ist NICHT aus Rechnungen geparst, sondern handschriftlich gezaehlt.

Beispiel Snickers: `init=77` — du hast 77 Snickers am 02.05. gezaehlt.
Laut Rechnungen wurden insgesamt nur 64 geliefert (32@24.09. + 32@28.02.).
Differenz +13: entweder gab es weitere unbekannte Lieferungen vor Sep 2025,
oder Zaehlfehler im Bereich Snickers.

Beispiel Snickers Creamy: `init=23` — Rechnung 22.04. = 24. Differenz -1:
1 Stueck zwischen 22.04. und 02.05. verkauft (vor Cutover, nicht in Tx-Log).

### Inventur-Diagnose (`guv_check_tmp/inventory_diagnose.js`)

Aktuell sind die Tx und Lagercharge-Buchungen MATHEMATISCH konsistent
(alle 19 Tests gruen). ECHTER SCHWUND wurde in 3 Produkten erkannt:

| SKU | initial (Charge) | sollte sein | ist | Schwund |
|---|---|---|---|---|
| Falcone Nussnougat | 40 (07.03.) | 24 | 11 | **13 Stk** |
| Falcone Cranberry | 40 (07.03.) | 31 | 27 | **4 Stk** |
| Lichtenauer Medium | 18 (02.05.) | 14 | 11 | **3 Stk** |

### Inventur-Tools (`guv_check_tmp/inventory_*.js`)

Da die `initial_qty` nicht aus Rechnungen rekonstruierbar ist, wurde ein
Inventur-Reset-Mechanismus gebaut:

- **`inventory_export.js [suffix]`** legt im Google Sheet einen neuen Tab
  `Inventur_<datum><suffix>` an mit allen aktiven Produkten. Spalten:
  `product_key | internal_product_name | mdb_codes | slot_count |
   slot_capacity_sum | aktuell_im_slot | aktuell_total_buch |
   real_im_slot | real_total | bemerkung`.

- **`inventory_apply.js [suffix] [--dry-run]`** liest den Tab und appliert
  die ausgefuellten `real_total` (+ optional `real_im_slot`) auf:
  - Lagerchargen: juengste aktive Charge bekommt `remaining=real_total`
    (und ggf. `initial_qty` erhoeht, falls real > initial); aeltere
    Chargen werden auf `remaining=0, status=leer` geschlossen.
  - Produkte: `current_machine_qty = real_im_slot` (oder MIN(real_total, capacity))
    fuer jeden zugeordneten Slot.
  - Audit-Eintrag in `Fehler_und_Hinweise` (type=INVENTUR_RESET).

Im Sheet liegt bereits `Inventur_2026-05-13_v2` mit aktuellen Werten.
User fuellt nur die Spalten `real_total` (+ optional `real_im_slot`) aus
und ruft `node inventory_apply.js _v2` auf.

### Erweiterte Test-Suite (19 Tests, alle gruen)

Neue Tests:
- T08d: keine Lagercharge mit `remain > initial` (Buchungs-Overflow)
- T08e: keine negativen `remaining_qty`
- T08f: Rechnungs-Chargen haben `initial_qty = Rechnungs-Lieferungsmenge`

### Vorheriger Teil derselben Session

### Wichtigster Bugfix dieser Session: `Produkte lesen` Filter

**Problem 7 (kritisch):** Trotz aller Code-Fixes liefen neue Verkaufslaeufe
weiter mit `UNKNOWN_PRODUCT`. Root-Cause war NICHT der FIFO-Code, sondern
der vorgelagerte Sheet-Read-Node:

```
Google Sheets - Produkte lesen
  filtersUI: [
    { lookupColumn: 'active',     lookupValue: 'TRUE' },
    { lookupColumn: 'machine_id', lookupValue: '{{ $json.MachineID }}' }
  ]
```

Der Filter `machine_id = MachineID` filterte **alle** Produkte mit
Nayax-Sub-Slot-IDs (457107529–575) weg. Nur Produkte mit `machine_id=457107528`
kamen durch — das war NUR 1 Stueck. Mein vorheriger FIFO-Code-Fix (tolerantes
Slot-Match) wirkte daher nicht, weil die Daten bereits gefiltert ankamen.

**Fix:** machine_id-Filter aus dem Read-Node entfernt
(`guv_check_tmp/patch_wf3_read_filter.js`). Active-Filter bleibt.
Der FIFO-Code filtert ohnehin tolerant via `getMainMachineFromSlot`.

**Verifikation:** WF3 Execution 488 liefert jetzt **36 Items** aus
`Google Sheets - Produkte lesen` (vorher: 1). Alle 16 Test-Suite-Tests gruen.

### Erweiterung: Batch-ID-Backfill fuer Pre-A2 OK-Tx

48 historische OK-Tx vom 2026-05-04 bis 2026-05-08 hatten `status=OK` +
`product_key`, aber kein `batch_id_abgebucht` (Feld wurde erst in Phase A2
hinzugefuegt). Per FIFO retroaktiv zugeordnet
(`guv_check_tmp/backfill_batch_id.js` + `backfill_apply.js`).

### Test-Suite (16 Tests, alle gruen)

`guv_check_tmp/tests/test_suite.js` — laeuft idempotent als
Stabilitaets-Check. Inhalt:

**Daten-Konsistenz:**
- T01 Produkte aktive haben pk + mdb + slot_id
- T02 product_slot_id beginnt mit `PS_457107528_`
- T03 Lagerchargen aktiv+remain>0 haben pk + mhd
- T04 OK-Tx haben pk + batch_id_abgebucht
- T05 keine UNKNOWN_PRODUCT-Tx ueber aktive Slot+Name-Treffer
- T06 Tx product_keys existieren in Produkte
- T07 jeder MDB nur einen active Slot
- T08 keine Doppelbuchung (OK-Tx pro Charge ≤ initial_qty)
- T08b Info: Schwund-Indikator (kein Failure)
- T08c keine doppelten transaction_id

**Workflow-Konfiguration:**
- T09a FIFO-Code: slot-basiert (`getMainMachineFromSlot`)
- T09b FIFO-Code: tolerante Regex `(=|\s+)`
- T10 `Produktbestand Update`: kein `product_key="="`
- T11 `Produkte lesen` filtert NICHT auf machine_id

**Smoke-Tests:**
- T12 Dashboard erreichbar
- T13 Letzter WF3-Run: status=success

### Vorheriger Teil derselben Session (Catch-up + WF5)

### Erweiterung: Produktbestand-Update + Dashboard-Trigger gefixt

**Problem 5: WF3 `Produktbestand Update` schrieb product_key auf leer**

Im Node `Produktbestand Update` (Google Sheets, operation=update) stand:
```
columns.value: { product_key: "=" }
```

Eine leere n8n-Expression. Beim Update wurde versucht, die product_key-Spalte
mit leerem String zu ueberschreiben. n8n hat das offenbar in vielen Faellen
ignoriert, aber das ist riskant — beim naechsten n8n-Update koennte es
product_key auf '' setzen.

Fix:
- `columns.value.product_key` entfernt
- `columns.schema[product_key].removed = true` gesetzt
- Patch-Skript: `guv_check_tmp/patch_wf3_produktbestand_update.js`

Die anderen Update-Felder bleiben unveraendert (current_machine_qty,
last_change_id, notes, product_slot_id [als matching]).

**Problem 6: Dashboard "Aktion konnte nicht gestartet werden" bei allen Webhook-Workflows**

Root-Cause:
- n8n Webhook-Trigger default httpMethod = **GET** (typeVersion 2+)
- Dashboard `server.js` hatte:
  - `method: clean(node.parameters?.httpMethod || 'POST').toUpperCase()` → falscher Default POST
  - Trigger-Endpoint rief Webhook hardcoded mit POST auf
- Resultat: 404 "This webhook is not registered for POST requests. Did you mean to make a GET request?"

Fix (`dashboard/server.js`):
- Webhook-Default-Method auf 'GET' geaendert
- Neue Funktion `firstProductionWebhook(workflow)` liefert den Node inkl. method
- Action-Objekte exponieren `primaryMethod`
- Trigger-Endpoint nutzt jetzt korrekt:
  - GET → Payload via query-string
  - POST/PUT/PATCH → JSON-Body
- Bessere Fehler-Meldungen mit `message`-Feld
- Robust gegen Netzwerkfehler (try/catch um fetch)

Verifikation (alle 5 Actions geprueft):
- ✅ invoice-intake (WF1, webhook GET) → 200 "Workflow was started"
- ✅ invoice-approval (WF2, form) → URL geoeffnet
- ✅ sales-fifo (WF3, webhook GET) → 200 "Workflow was started"
- ✅ slot-assignment (WF4, form) → URL geoeffnet
- ✅ mhd-stock-check (WF5, webhook GET) → 200 "Workflow was started"

**End-to-End Validierung WF3:**
- WF3 via Dashboard getriggert: execution 480 success
- 172 Sales empfangen, 0 neue Tx (Wasserzeichen aktuell — keine Aktionen noetig)
- E2E-Logik-Test (`guv_check_tmp/e2e_test_fifo.js`): Coca Cola Zero
  Match → product_key=SKU_COCA_COLA_ZERO, batch=B_COCA_COLA_ZERO_20260502_1,
  Updates wuerden korrekt 3→2 (current) + 3→2 (remain) machen
- Alle 3 WF3-Update-Nodes (`Produktbestand Update`,
  `Google Sheets - Lagerchargen aktualisieren`,
  `Google Sheets - Transaktionen anhängen`) geprueft: sauber konfiguriert,
  matching-Cols korrekt, keine Bug-Expressions mehr

### Session 6 – Initialer Teil: WF3 Machine-Match-Fix + Catch-up + WF5 Empty-Detection

### Was diese Session passiert ist

Bevor mit Phase 2 (historische Daten) weitergemacht wurde, wurden mehrere
Stabilitaetsprobleme entdeckt + gefixt:

**Problem 1: product_key fehlt bei neuen Verkaeufen (111 von 161 Tx)**

Root-Cause: Jede aktive Zeile in `Produkte` hat eine andere `machine_id`
(457107528–457107575 — Nayax-Sub-Slot-IDs). Die Nayax-API liefert fuer
Verkaeufe aber nur die Haupt-MachineID `457107528`. WF3's
`activeProductsForMachine(machineId)` machte einen strikten Vergleich
`p.machine_id === machineId` → kein Match → `product_key=''`,
`batch_id_abgebucht=''`, keine FIFO-Abbuchung.

Fix in `Code - FIFO berechnen` + `Code in JavaScript`:
- Neue Hilfsfunktion `getMainMachineFromSlot(p)` extrahiert die
  Haupt-MachineID aus dem `product_slot_id` (Pattern `PS_<machineId>_...`).
- `activeProductsForMachine` matched jetzt tolerant ueber:
  Haupt-MachineID aus slot_id ODER direkter machine_id-Match ODER leeres
  machine_id (Fallback wie zuvor).
- `parseProductName` Regex akzeptiert sowohl `Name(MDB  Preis)` (zwei Spaces)
  als auch `Name(MDB = Preis)` (mit Gleichheitszeichen).
- Patch-Skript: `guv_check_tmp/patch_wf3_machine_match.js`
- Backup: `_wf3_BACKUP_2026-05-13T18-07-17-852Z.json`

**Problem 2: 111 unmatched Tx historisch zuordnen**

Catch-up-Skript: `guv_check_tmp/catchup_compute.js` (Berechnung, dry-run) +
`catchup_apply.js` (ad-hoc Workflow erstellt) + `catchup_execute.js`
(Webhook-getriggert + cleanup).

Ergebnis:
- 82 Tx (von 111) konnten zugeordnet werden:
  - 41 davon mit `status=OK` + FIFO-Abbuchung
  - 41 mit `status=INSUFFICIENT_BATCH_STOCK` — sind Tx VOR der
    Bestandsaufnahme 02.05. ohne passende historische Lagercharge
    (wird in Phase 2 mit historischen Chargen aufgeloest)
- 15 Lagerchargen `remaining_qty` korrekt reduziert
- 17 Slots `current_machine_qty` korrekt reduziert
- 29 Tx bleiben unmatched (historische Produkte: Twix Original auf MDB 14,
  Bueno White auf MDB 45, M&M's auf MDB 64, Pombaeren auf MDB 30, Salzstangen
  auf MDB 24, Mr.Toms auf MDB 47 — alle nicht mehr aktiv auf diesen Slots)
- Backup vor Writes: `guv_check_tmp/backup_2026-05-13T18-11-47-652Z/`
- Plan: `guv_check_tmp/_catchup_plan.json`

**Problem 3: WF5 false-positive "Red Bull Spring leer" + Cola Zero nicht erkannt**

Root-Cause: WF5 alarmiert nur ueber `current_machine_qty < min_stock`.
Wenn `current_machine_qty` stale ist (weil WF3 nicht abgebucht hat),
sind die Alerts unzuverlaessig. Cola Zero stand auf `current_qty=4`
(real leer/3), Red Bull Spring stand auf `current_qty=0` (real voll).

Fix WF5 `Code - MHD und Lagercharge pruefen`:
- Neue zusaetzliche Aggregat-Pruefung VOR der Schleife:
  - `EMPTY_BATCH` (severity: critical) wenn Summe(remaining_qty) per
    product_key <= 0
  - `LOW_BATCH` (severity: warning) wenn Summe <= `low_batch_qty_threshold`
    (Config, default 5)
- LOW_STOCK-Severity automatisch auf `critical`, wenn `current_machine_qty = 0`
- Patch-Skript: `guv_check_tmp/patch_wf5_empty_detection.js`
- Backup: `_wf5_BACKUP_<ts>.json`

**Problem 4: Manuelle Inventur-Korrekturen**

Per User-Eingabe:
- `SKU_RED_BULL_SPRING` (MDB 54): `current_machine_qty=5`, `machine_capacity=5`,
  `target_stock=5` (war 0 — Slot ist real voll)
- `SKU_LICHTENAUER_STILL` (MDB 47): `machine_capacity=5`, `target_stock=5`
  (war 0/0 — WF4-Bug bei Slot-Wechsel)
- Fix-Skript: `guv_check_tmp/fix_inventory_manual.js`

**Aufraeumen:**
- Alle `WF_TEMP_*`-Workflows geloescht (`cleanup_temp_wfs.js`):
  - WF_TEMP_CATCHUP_2026-05-13T18-11-47 (`4Z278fpr9p14nX1L`)
  - WF_TEMP - Update GuV Daten (`m6hTE6lCQfdefP15`) — Reststand von Session 5

### Stand der Sheet-Daten nach Fix (Drink-Fokus)

| product_key | current_qty | remaining_qty | Anmerkung |
|---|---|---|---|
| SKU_COCA_COLA_ZERO | 0 | 3 | Slot leer, 3 im Backstock |
| SKU_COCA_COLA | 3 | 22 | normal |
| SKU_RED_BULL | 1 | 5 | normal |
| SKU_RED_BULL_SPRING | 5 | 12 | manuell auf voll gesetzt |
| SKU_FANTA_EXOTIC | 5 | 22 | normal |
| SKU_SPRITE | 4 | 22 | normal |
| SKU_LICHTENAUER_MEDIUM (MDB 50/46) | 2 / 5 | 12 (1 Charge) | normal |
| SKU_LICHTENAUER_STILL (MDB 57/47) | 2 / 0 | 13 (1 Charge) | MDB47 jetzt capacity=5 |

### Naechster Schritt (laut Plan)

Phase 2 Implementation Schritte 1–4 fortsetzen
(siehe Block weiter unten "Phase 2 Datenerfassung abgeschlossen 2026-05-12"):

1. WF_TEMP2 — 16 neue SKUs (historisch, active=FALSE) + 60 historische Chargen anlegen
2. Lokaler FIFO-Algorithmus ueber 518 historische Nayax-Verkaeufe
3. WF_TEMP3 — 518 historische Verkaufseintraege in Verarbeitete_Transaktionen
4. WF8 Re-Run fuer komplette GuV ab Oktober 2025

### Hinweis fuer naechsten Verkaufslauf

Mit dem WF3-Patch (slot-basiertes Matching) werden zukuenftige Nayax-Verkaeufe
korrekt zugeordnet, auch wenn `Produkte.machine_id` Sub-Slot-IDs enthaelt.
Kein Re-Sync der bestehenden Verkaeufe noetig.

Der `inventory_cutover_datetime` aus dem `Config`-Node + Watermark
`last_inventory_review_at` aus `Workflow_State` schuetzen vor Doppel-Abbuchung
bei den 41 historisch erkannten INSUFFICIENT_BATCH_STOCK-Tx (sie sind bereits
mit `transaction_id` in Verarbeitete_Transaktionen und werden via
`processed.has()` uebersprungen).
