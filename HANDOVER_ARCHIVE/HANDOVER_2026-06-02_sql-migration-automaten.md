...
- **Heute-Reiter: Bestands-Warnungen mit Live-Zahl (automatenlager `aca0f5e`, live):** Die `warnings`-Tabelle trägt einen von WF5 **eingefrorenen** Meldungstext (z. B. „Red Bull Spring: Nur noch 5 Stück im Lager"), obwohl längst leer. Das Cockpit prüfte die Bedingung zwar live (`LIVE_WARNING_RECONCILE_SQL`), zeigte aber die alte Zahl. Fix in `overview-monitoring.js`: die Warnungs-Query baut den Text für `LOW_BATCH`/`LOW_STOCK` aus dem **aktuellen PG-Backstock** neu (`GREATEST(SUM(remaining_qty)−SUM(current_machine_qty),0)`, wie inventory-mhd/digest) → zeigt jetzt „Red Bull Spring: Lager leer (0 im Lager, 1 im Automat)". MHD-/System-Warnungen unverändert; keine Daten geändert. Test AC-LIVEMSG.

**Tests:** 689/689 (`cd dashboard; npm test`). Repo↔Mini synchron (WF-JSONs exportiert).
