...
**✅ ERLEDIGT (2026-06-11) — A2/Stufe 6 „n8n-Ablösung" KOMPLETT (Commit `2ff12e9`):** Alle 18 n8n-Workflows deaktiviert (letzte Ports: WF1-Upload→Drive, WF4-Produktwechsel atomar durch die Tür, Nayax-Abgleich direkt gegen die API), **Migration 0033 angewendet** (`n8n_app` verliert BYPASSRLS) → **RLS systemweit ohne Ausnahme**. Suite 1337/1338 (1 skip gegenstandslos). Abschlussprotokoll: `docs/audit/n8n-abloesung-abschluss-2026-06-11.md`. Offen nur noch Aufräumarbeiten (n8n-Container stilllegen, Webhook-Fallbacks entfernen, cutover-monitor aus dem Schedule).

**Geplant (2026-06-11) — NÄCHSTE PHASE: Cloud-Migration (Phase B) + Betriebsreife (A3) cloud-nativ:** SPEC `docs/specs/cloud-migration-3-schichten-phase-b-v1.md`, gegen **echten Code** verifiziert (Explore-Pass). **Umsetzung durch Model Fable** → bewusst gröber/slice-orientiert. Inkrementeller Umzug Mini → **Supabase** (DB+RLS+**Auth**) · **Render** (Backend+Jobs) · **Cloudflare** (Frontend+Domain) in 6 Slices (Fundament/Domain → DB → Auth-Naht → Backend → Frontend → Betriebsreife/Cutover), kurzes Wartungsfenster, Mini als Rollback. Kernentscheidungen: **Supabase Auth** (JWT ersetzt Tailscale-Header, RLS-Mandanten-Tür bleibt unverändert); **Start auf Gratis-Stufen** (Cron via pg_cron/Cloudflare statt Render-Cron; Off-Site-`pg_dump`+Alarm statt Supabase-Auto-Backup); neue Domain via Cloudflare Registrar. Verifizierte Supabase-Fallstricke: GUC `automatenlager.current_tenant` vorregistrieren (sonst RLS 42704); **kein Custom-BYPASSRLS** → Infra-Pool=`service_role`-Äquiv., App-Pool=`automatenlager_app`-Äquiv.; Migration 0033 bedingt machen (n8n_app fehlt); **flüchtiges Render-FS** → Audit-/Guest-Access-Log + `.dashboard-config.json` in die DB/Env. Login bewusst minimal — **ordentliches Design = technische Schuld für Phase C** (vermerkt). Out of Scope: 2. Kunde, Billing, Marketing-Site, neue Features, volles CI/CD. **Issues geschnitten: [#212](https://github.com/PatrickM-git/automatenlager/issues/212)–[#219](https://github.com/PatrickM-git/automatenlager/issues/219)** entlang der 6 Slices (#212 Fundament/Domain · #213 FS-Fix Audit-Log→DB · #214 DB→Supabase · #215 Auth-Naht · #216 Off-Site-Backup · #217 Backend→Render · #218 Frontend→Cloudflare · #219 Cutover-Abschluss). **Nächster Schritt: neuer Chat → `start-issue`** (Slice 0 = #212 zuerst; #213 ist parallel ohne Blocker), dann `tdd`.
