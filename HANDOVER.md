...
- **Neuer Single-Source-Endpunkt `GET /api/v2/alerts/digest` (automatenlager `e8372e2`, live):** `dashboard/lib/alert-digest.js` berechnet alle Mail-Sektionen aus **PG-Fakten** — MHD (abgelaufen/bald) aus `stock_batches`, Lagerchargen leer/niedrig als Aggregat je Produkt, „Niedriger Bestand" = leere aktive Slots (`slot_assignments.current_machine_qty=0`), und **Daten-/Workflowfehler nur noch type+severity-basiert** (operative Warnungstypen + fehlgeschlagene `audit.workflow_runs`). Bestand/MHD bewusst aus Fakten, NICHT aus der `warnings`-Tabelle (die WF5 selbst schreibt → keine Schleife). Tests `dashboard-alert-digest.test.js` (AD1–AD6 + HTTP-503).
